  ...
  const projects = useMemo<Project[]>(
    () => [
      ...
      },
      {
        ...
        longDescription:
          'Crafted with Emergent Agent and Claude Sonnet, this developer-themed portfolio delivers a polished experience that highlights personal brand, recent work, and contact pathways.',
        image: developerThumbnail,
        images: [developer1, developer2, developer3, developer4, developer5, developer6],
          ...
          <DialogContent className="sm:max-w-2xl">
            ...
            {activeProject && (
              <div className="space-y-4">
                ...
                <p className="text-muted-foreground">{activeProject.longDescription}</p>
                <div>
                  <h4 className="mb-2 text-foreground">{t('projects.technologiesUsed')}</h4>
                  <div className="flex flex-wrap gap-2">
                    {activeProject.technologies.map((tech) => (
                      <Badge key={`${activeProject.title}-detail-tech-${tech}`} variant="secondary">
                        {tech}
                      </Badge>
                    ))}
                  </div>
                ...
                <div>
                  <h4 className="mb-2 text-foreground">{t('projects.keyFeatures')}</h4>
                  <ul className="grid gap-2 md:grid-cols-2">
                    {activeProject.features.slice(0, 8).map((feature, index) => (
                      <li
